...
import { OnCharacterAdded, OnCharacterRemoving, OnPlayerAdded, OnPlayerRemoving } from "shared/lifecycles";

@Service()
export class LifecycleService implements OnStart {
	public onStart(): void {
		const playerAddedListeners = new Set<OnPlayerAdded>();
		Modding.onListenerAdded<OnPlayerAdded>((obj) => playerAddedListeners.add(obj));
		Modding.onListenerRemoved<OnPlayerAdded>((obj) => playerAddedListeners.delete(obj));
		...
		Modding.onListenerAdded<OnCharacterAdded>((obj) => characterAddedListeners.add(obj));
		Modding.onListenerRemoved<OnCharacterAdded>((obj) => characterAddedListeners.delete(obj));

		const characterRemovingListeners = new Set<OnCharacterRemoving>();
		Modding.onListenerAdded<OnCharacterRemoving>((obj) => characterRemovingListeners.add(obj));
		Modding.onListenerRemoved<OnCharacterRemoving>((obj) => characterRemovingListeners.delete(obj));
		...
		Players.PlayerAdded.Connect((player) => {
			...
			player.CharacterRemoving.Connect((character) => {
				for (const otherListener of characterRemovingListeners) {
					task.spawn(() => otherListener.onCharacterRemoving(character));
				}
			});
		});
